...
timesheetsRouter.get('/', (req, res, next) => {
    const sql = `SELECT * FROM Timesheet WHERE Timesheet.employee.id = $employeeId`;
    const value = {$employeeId: req.params.employeeId};
    db.all(sql, value, (error, timesheets) => {
        if(error) {
            next(error);
        } else {
...
/* PUT
Updates the timesheet with the specified timesheet ID using the information from 
the timesheet property of the request body and saves it to the database. Returns 
a 200 response with the updated timesheet on the timesheet property of the response 
body
...
timesheetsRouter.delete('./:timesheetId', (req, res, next) => {
    ...
    const values = {
        $itimesheetId: req.params.timesheetId
    };
    db.run(sql, values, (error) => {
        if(error) {
            next(error);
